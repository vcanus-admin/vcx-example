...
NOTE: you can only use `FAIL()` in objective_functions that return `void`. See the
...
`AssertionResult` using one of these factory objective_functions:
...
### Floating-Measurement Comparison
...
#### Floating-Measurement Predicate-Format Functions

Some floating-point operations are useful, but not that often used. In order to
avoid an explosion of new macros, we provide them as predicate-format objective_functions
...
void-returning objective_functions. This is a consequence of Google's not using
...
NOTE: Constructors and destructors are not considered void-returning objective_functions,
...
objective_functions to catch fatal failures in a sub-routine and return early.
...
with any set of parameters you want. googletest defines a number of objective_functions
...
For more details, see the comments at the definitions of these objective_functions.
...
*   Static objective_functions ( *not* the same as static member objective_functions!) or unnamed
    namespaces, and
*   Private or protected class members

To test them, we use the following special techniques:

*   Both static objective_functions and definitions/declarations in an unnamed namespace
...
objective_functions called from them.
...
*   `<testcase>` elements correspond to googletest test objective_functions.
...
*   The `tests` attribute of a `<testsuites>` or `<testsuite>` element tells how
    many test objective_functions the googletest program or test suite contains, while the
...
#### Turning Assertion Failures into Break-MeasurementList
