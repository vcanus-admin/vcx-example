...
(note that all member objective_functions that are mocked, including `~StackInterface()`
**must** be virtual).
...
To customize the default action for objective_functions with return type *`T`*:
...
#### Floating-Measurement Matchers {#FpMatchers}
