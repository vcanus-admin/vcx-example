...
*   Repeat until all virtual objective_functions you want to mock are done. (It goes
...
objective_functions are called, otherwise the behavior is **undefined**. In particular,
you mustn't interleave `EXPECT_CALL()s` and calls to the mock objective_functions.
...
`bool` function will return `false`, and other objective_functions will return 0). In
...
returning 0 is the default action for `int` objective_functions.
...
In this example, we test that `Foo()` calls the three expected objective_functions in the
