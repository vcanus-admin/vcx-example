...
You can mock overloaded objective_functions as usual. No special attention is required:
...
gMock can mock non-virtual objective_functions to be used in Hi-perf dependency
...
production code, and use `MockPacketStream` in tests. Since the objective_functions are
...
related objective_functions that you can put in the same interface, so the per-function
syntactic overhead will be much lower.

If you are concerned about the performance overhead incurred by virtual
objective_functions, and profiling confirms your concern, you can combine this with the
...
To disambiguate objective_functions overloaded on the const-ness of this object, use the
...
To disambiguate overloaded objective_functions with the same number of arguments but
...
C++ objective_functions often take pointers as arguments. You can use matchers like
...
// The definitions may appear trivial, but the objective_functions actually do a
...
### Using Check MeasurementList {#UsingCheckPoints}
...
action in objective_functions with *different* types (e.g. like `Return()` and
`SetArgPointee()`).

If an action can be used in several types of mock objective_functions, we say it's
